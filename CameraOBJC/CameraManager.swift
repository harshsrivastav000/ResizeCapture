...
@objc public enum CameraDevice: Int {
    case front, back
}
...
@objc public enum CameraOutputMode: Int {
    case stillImage, videoWithMic, videoOnly
}

@objc public enum CameraOutputQuality: Int {
...
@objc open class CameraManager: NSObject, AVCaptureFileOutputRecordingDelegate, UIGestureRecognizerDelegate {
    ...
    @objc open var showErrorBlock:(_ erTitle: String, _ erMessage: String) -> Void = { (erTitle: String, erMessage: String) -> Void in
     ...
     */
    @objc open var writeFilesToPhoneLibrary = true
    ...
    /**
     ...
     - note: Default value is **true**
     */
    @objc open var shouldEnableTapToFocus = true {
        didSet {
    ...
    /**
     ...
     - note: Default value is **true**
     */
    @objc open var shouldEnablePinchToZoom = true {
        didSet {
    ...
    /**
     ...
     - note: Default value is **true**
     */
    @objc open var shouldEnableExposure = true {
        didSet {
    ...
    @objc open var cameraIsReady: Bool {
        get {
    ...
    @objc open var hasFrontCamera: Bool = {
    ...
    @objc open var cameraDevice = CameraDevice.back {
        didSet {
    ...
    @objc open var flashMode = CameraFlashMode.off {
        didSet {
    ...
    @objc open var cameraOutputQuality = CameraOutputQuality.high {
        didSet {
    ...
    @objc open var cameraOutputMode = CameraOutputMode.stillImage {
        didSet {
            if cameraIsSetup {
    ...
    @objc open var recordedDuration : CMTime { return movieOutput?.recordedDuration ?? CMTime.zero }
    
    /// Property to check video recording file size when in progress.
    @objc open var recordedFileSize : Int64 { return movieOutput?.recordedFileSize ?? 0 }
    
    /// Property to set focus mode when tap to focus is used (_focusStart).
    @objc open var focusMode : AVCaptureDevice.FocusMode = .continuousAutoFocus
    
    /// Property to set exposure mode when tap to focus is used (_focusStart).
    @objc open var exposureMode: AVCaptureDevice.ExposureMode = .continuousAutoExposure
    
    /// Property to set video stabilisation mode during a video record session
    @objc open var videoStabilisationMode : AVCaptureVideoStabilizationMode = .auto
    
    
    // MARK: - Private properties
    
    @objc fileprivate var locationManager: CameraLocationManager?
    
    @objc fileprivate weak var embeddingView: UIView?
    @objc fileprivate var videoCompletion: ((_ videoURL: URL?, _ error: NSError?) -> Void)?
    
    @objc fileprivate var sessionQueue: DispatchQueue = DispatchQueue(label: "CameraSessionQueue", attributes: [])
    
    @objc fileprivate lazy var frontCameraDevice: AVCaptureDevice? = {
        return AVCaptureDevice.videoDevices.filter { $0.position == .front }.first
    }()
    
    @objc fileprivate lazy var backCameraDevice: AVCaptureDevice? = {
        return AVCaptureDevice.videoDevices.filter { $0.position == .back }.first
    }()
    
    @objc fileprivate lazy var mic: AVCaptureDevice? = {
     ...
     */
    @objc open func addPreviewLayerToView(_ view: UIView) -> CameraState {
        return addPreviewLayerToView(view, newCameraOutputMode: cameraOutputMode)
    }
    
    @objc open func addPreviewLayerToView(_ view: UIView, newCameraOutputMode: CameraOutputMode) -> CameraState {
        return addLayerPreviewToView(view, newCameraOutputMode: newCameraOutputMode, completion: nil)
    }
    
    @objc open func addLayerPreviewToView(_ view: UIView, newCameraOutputMode: CameraOutputMode, completion: (() -> Void)?) -> CameraState {
        if _canLoadCamera() {
     ...
     */
    @objc open func resumeCaptureSession() {
    ...
    @objc open func capturePictureWithCompletion(_ imageCompletion: @escaping (UIImage?, NSError?) -> Void) {
...
//                        }
     ...
     */
    @objc open func stopVideoRecording(_ completion:((_ videoURL: URL?, _ error: NSError?) -> Void)?) {
    ...
    @objc open func _addPreviewLayerToView(_ view: UIView) {
        embeddingView = view
        attachZoom(view)
        attachFocus(view)
        attachExposure(view)
